...
function buildMoveFix({ property, destination, sourceCode }) {
  ...
  const hasTrailingComma = trailingToken.value === ',';
  const removal = hasTrailingComma
    ? [previousToken.range[1], trailingToken.range[1]]
    : [previousToken.range[0], property.range[1]];
  const separator =
    destination.loc.start.line === property.loc.start.line
      ? ' '
      : `\n${getIndentBefore(destination, sourceCode)}`;

  return (fixer) => [
    fixer.insertTextBefore(destination, `${sourceCode.getText(property)},${separator}`),
    ...
    return {
      ':function'(node) {
        ...
        stateObject.properties
          ...
          .forEach((property) => {
            const isMovable =
              !hasSpread && canMove(property, sourceCode) && canMove(firstAction, sourceCode);
            ...
            context.report({
              ...
              fix: isMovable
                ? buildMoveFix({ property, destination: firstAction, sourceCode })
                : undefined,
            });
          });
      },
