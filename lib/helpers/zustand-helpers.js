...
const SHALLOW_MODULE = 'zustand/react/shallow';
const SHALLOW_MODULE_PATTERN = /^zustand(?:\/react)?\/shallow$/u;
...
function describeStateRead(node) {
  const expression = unwrapExpression(node);

  if (expression?.type === 'Identifier') {
    return { rootName: expression.name, hasComputedSegment: false };
  }
  if (expression?.type === 'ChainExpression') return describeStateRead(expression.expression);
  if (expression?.type !== 'MemberExpression') return undefined;

  const object = describeStateRead(expression.object);

  return object
    ? { rootName: object.rootName, hasComputedSegment: object.hasComputedSegment || expression.computed }
    : undefined;
}
...
function bindsUseShallow(statement) {
  return (
    statement.type === 'ImportDeclaration' &&
    statement.specifiers.some((specifier) => specifier.local.name === 'useShallow')
  );
}

function getAppendableShallowImport(imports) {
  return imports.find((statement) => {
    if (!SHALLOW_MODULE_PATTERN.test(statement.source.value)) return false;
    const lastSpecifier = statement.specifiers[statement.specifiers.length - 1];
    return lastSpecifier?.type === 'ImportSpecifier';
  });
}

function usesSemicolons(imports, sourceCode) {
  if (imports.length === 0) return true;
  return imports.some((statement) => sourceCode.getLastToken(statement).value === ';');
}

function getShallowImportFixes(sourceCode, fixer) {
  if (sourceCode.ast.body.some(bindsUseShallow)) return [];

  const imports = sourceCode.ast.body.filter(
    (statement) => statement.type === 'ImportDeclaration'
  );
  const appendable = getAppendableShallowImport(imports);

  if (appendable) {
    const lastSpecifier = appendable.specifiers[appendable.specifiers.length - 1];
    return [fixer.insertTextAfter(lastSpecifier, ', useShallow')];
  }

  const terminator = usesSemicolons(imports, sourceCode) ? ';' : '';
  const declaration = `import { useShallow } from '${SHALLOW_MODULE}'${terminator}`;
  const lastImport = imports[imports.length - 1];

  return lastImport
    ? [fixer.insertTextAfter(lastImport, `\n${declaration}`)]
    : [fixer.insertTextBeforeRange([0, 0], `${declaration}\n`)];
}
